...
class RepaymentDetailResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        return [
            ...
            'debts' => $this->whenLoaded('debts', function () {
                return $this->debts->map(function ($debt) {
                    return [
                        'id' => $debt->id,
                        'original_amount_fcfa' => $debt->original_amount_fcfa,
                        'remaining_amount_fcfa' => $debt->remaining_amount_fcfa,
                        'status' => $debt->status,
                        'amount_applied_fcfa' => $debt->pivot->amount_applied_fcfa,
                        'incurred_at' => $debt->incurred_at,
                    ];
                });
            }),
